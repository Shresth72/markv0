...
NS_LOG_COMPONENT_DEFINE("RandomBurstTrafficSimulation");

int main(int argc, char *argv[]) {
  // Enable logging for TCP and UDP
  LogComponentEnable("RandomBurstTrafficSimulation", LOG_LEVEL_INFO);
  ...
  // Create random burst traffic on node 0 (client)
  OnOffHelper client("ns3::TcpSocketFactory", sinkAddress);

  // Simulate random burst traffic using Pareto distribution
  client.SetAttribute(
      "OnTime", StringValue("ns3::ParetoRandomVariable[Mean=0.5,Shape=1.5]"));
  client.SetAttribute(
      "OffTime", StringValue("ns3::ParetoRandomVariable[Mean=0.5,Shape=1.5]"));

  // Set data rate and packet size for bursts
  client.SetAttribute("DataRate",
                      StringValue("10Mbps")); // Simulate high bursts
  ...
  logFile.open("random_burst_traffic_simulation_log.txt");
  ...
  logFile.close();
  flowMonitor->SerializeToXmlFile("random_burst_flow_monitor.xml", true, true);
