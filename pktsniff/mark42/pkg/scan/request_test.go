		...
		},
		{
			...
			expected: []interface{}{
				newScanRequest(
					withDstIP(nil),
					withDstPort(888),
					withError(&ipError{errors.New("ip error")}),
				),
			},
		},
		{
...
func TestIPRequestGenerator(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			input: newScanRange(
				withSubnet(
					&net.IPNet{IP: net.IPv4(192, 168, 0, 1).To4(), Mask: net.CIDRMask(32, 32)},
				),
			),
			expected: []interface{}{
		...
		},
		{
			...
			input: newScanRange(
				withSubnet(
					&net.IPNet{IP: net.IPv4(192, 168, 0, 1).To4(), Mask: net.CIDRMask(31, 32)},
				),
			),
			expected: []interface{}{
		...
		},
		{
			...
			input: newScanRange(
				withSubnet(
					&net.IPNet{IP: net.IPv4(192, 168, 0, 1).To4(), Mask: net.CIDRMask(30, 32)},
				),
			),
			expected: []interface{}{
				newScanRequest(withDstIP(net.IPv4(192, 168, 0, 0).To4())),
			...
			go func() {
				...
				for i, filtered := range tt.filtered {
					if filtered {
						...
						excludeFilters = append(
							excludeFilters,
							gomock.Not(gomock.Eq(tt.input[i].DstIP)),
						)
					}
				}
				excludeIPs.EXPECT().
					Contains(gomock.All(excludeFilters...)).
					Return(false, nil).
					AnyTimes()
