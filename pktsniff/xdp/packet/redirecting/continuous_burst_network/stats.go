...
func collectStats2(xdpStatsMap *ebpf.Map, telemetryStatsMap *ebpf.Map) {
	startTime := time.Now()
	...
	for {
		...
		for i := 0; i < 5; i++ {
			...
			if i == 1 { // XDP_DROP
				...
				dropped += int64(totalRxPackets - int64(stats.RxPackets))
			}
		}
		...
		// Update for next RPS calculation
		...
		fmt.Printf(
			"Elapsed: %.2f, Dropped: %d, Passed: %d, ProcessingTime: %d, RPS: %.2f\n",
			elapsed,
			dropped,
			passed,
			processingTime,
